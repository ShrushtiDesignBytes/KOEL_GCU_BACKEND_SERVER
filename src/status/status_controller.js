...
module.exports = {
    ...
    createStatus: async (req, res) => {
        const { start_stop, auto_manual, breakeropen_close, reset, cooldown, timer, text, createdlocal_db, updatedlocal_db } = req.body;
        try {
            const data = await Status.create({
                start_stop, auto_manual, breakeropen_close, reset, cooldown, timer, text, createdlocal_db, updatedlocal_db
            });
